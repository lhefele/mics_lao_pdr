...
mics6.ch2 <- merge(mics6.ch, mics6.wm_var, by=c("HH1", "HH2", "UF4"), all.x = TRUE)

#subset children under 3
mics6.ch2 %>% filter(UB2 < 3) -> mics6.ch_b1 # 6
...
# Household ID
mics6.ch_b1 %>% unite(HH_id, c(HH1, HH2), sep = "_", remove = FALSE, na.rm = FALSE) -> mics6.ch_b1


# Are there any duplicates?
mics6.ch_b1 %>% group_by(HH_id) %>% filter(n()>1) %>% summarize(n=n())
...
# UF1 = HH1
# UF2 = HH2
mics6.ch_b1 %>% unite(ch_id, c(UF1, UF2, UF3), sep = "_", remove = FALSE, na.rm = FALSE) -> mics6.ch_b1

mics6.ch_b1 %>% group_by(ch_id) %>% filter(n()>1) %>% summarize(n=n()) # none...
# identify each child
...
mics6.ch_b1$UF7D_2 <- as.numeric(as.character(mics6.ch_b1$UF7D))
mics6.ch_b1$UF7Y_2 <- as.numeric(as.character(mics6.ch_b1$UF7Y))
mics6.ch_b1$UF7M_2 <- as.numeric(as.character(mics6.ch_b1$UF7M))

# day of interview
mics6.ch_b1 %>%  mutate(I_date = make_date(UF7Y_2, UF7M_2, UF7D_2)) -> mics6.ch_b1
...
# Date of birth 
mics6.ch_b1 %>% count(UB1D)
mics6.ch_b1 %>% count(UB1M)

class(mics6.ch_b1$UB1M)

mics6.ch_b1$UB1D_2 <- as.numeric(mics6.ch_b1$UB1D)

mics6.ch_b1$UB1M_2 <- as.numeric(mics6.ch_b1$UB1M)

mics6.ch_b1$UB1Y_2 <- as.numeric(mics6.ch_b1$UB1Y)

mics6.ch_b1 %>%  mutate(dob_ch = make_date(UB1Y_2, UB1M_2, UB1D_2)) -> mics6.ch_b1
mics6.ch_b1$dob_ch
class(mics6.ch_b1$UB1M_2)
...
mics6.ch_b1 %>% 
  mutate(IM11_IM12 = ifelse((IM11 == 2 | IM11 == 8) & 
                            (IM12A == 2 | IM12A == 8) &
                            (IM12B == 2 | IM12B == 8) &
                             (IM12C == 2 | IM12C == 8), 0, 1)) -> mics6.ch_b1
mics6.ch_b1 %>% count(IM11_IM12, IM11, IM12A, IM12B, IM12C)

mics6.ch_b1 %>% count(IM11, IM11_IM12)

mics6.ch_b1 %>% 
  mutate(IM11_2 = ifelse((IM11_IM12 == 0 & IM11 == 2), 2,
                  ifelse((IM11_IM12 == 0 & IM11 == 8), 8, NA))) -> mics6.ch_b1

mics6.ch_b1 %>% count(IM11,IM11_2)
# looks ok
...
mics6.ch_b1 %>% count(IM2, IM5, IM11) # all NAs in kids under 3 are recall; not NA
...
mics6.ch_b1 %>% mutate(recall = recode(IM11, "1" = 1, "2" = 1, "8" = 1)) -> mics6.ch_b1
                                  
mics6.ch_b1 %>% count(recall) # ok

# Replace all NA in children under 5 with "0"
mics6.ch_b1 %>%  mutate(recall = replace(recall, is.na(recall), 0))  -> mics6.ch_b1
mics6.ch_b1 %>% count(recall)

mics6.ch_b1 %>%count(IM11, recall) # seems like it did work
...
mics6.ch_b1 %>% filter(UB2 < 3) %>% count(IM2, IM5)
# First, need to convert NAs in IM5

mics6.ch_b1 %>% mutate(IM5_2 = recode(IM5,
                 "YES, ONLY CARD / MCH HANDBOOK SEEN" = 1,
                 "YES, ONLY OTHER DOCUMENT SEEN" = 2,
                 "YES, CARD / MCH HANDBOOK AND OTHER DOCUMENT SEEN" = 3,
                 "NO CARDS / MCH HANDBOOK AND NO OTHER DOCUMENT SEEN" = 4)) -> mics6.ch_b1

mics6.ch_b1 %>% mutate(IM5_2 = replace(IM5_2, is.na(IM5_2) & UB2 < 3, 5)) -> mics6.ch_b1
mics6.ch_b1 %>% count(IM5_2, recall)


# Variable 2
# Info from any document 
mics6.ch_b1 %>% mutate(IM5_3 = recode(IM5_2, "5" = 4)) -> mics6.ch_b1
mics6.ch_b1 %>% count(IM5_2, IM5_3) # ok

# recall variable IM11 in combination with IM5_2
mics6.ch_b1 %>% filter(UB2 < 3) %>% count(IM5_2, IM11)
...
mics6.ch_b1 %>%
  mutate(hepb0_R_R = recode(hepb0_R_1,
                           "R_0" = "0",
                           "R_8" = "0",
                           "R_9" = "0",
                           "R_Later" = "0",
                           "R_Yes" = "1")) -> mics6.ch_b1
mics6.ch_b1 %>% count(hepb0_R_R)
...
mics6.ch_b1 %>% 
  mutate(MN20_1 = recode(MN20,
                        ...
                        "98" = "9",
                        "99" = "9")) -> mics6.ch_b1
mics6.ch_b1 %>% count(MN20, MN20_1)
mics6.wm_var %>% count(MN20)

# new variable times received antenatal care: 1-3, 4-6, 7 or more
mics6.ch_b1 %>% 
  mutate(MN20_1 = recode(MN20,
                        "1" = "1",
                        "1" = "1",
...
mics6.ch_b1 %>% mutate(MN5_1 = if_else(MN5 >= 1 & MN5 <=3, 1,
                                        if_else(MN5 >= 4 & MN5 <= 6, 2,
                                        if_else(MN5 == 98, 9, 3)))) -> mics6.ch_b1
mics6.ch_b1 %>% count(MN5, MN5_1)
...
mics_design <- svydesign(data = mics6.ch_b1, 
                         weights = ~chweight, 
                         id = ~strata+HH1)
mics_design
...
mics_design1 <- svydesign(data = mics6.ch_p1, 
                         weights = ~chweight, 
                         id = ~strata+HH1)
mics_design1
...
tab_IM6PENTA1D <- svytable(~IM6PENTA1D, design = mics_design1) %>% 
  as.data.frame() %>% 
  mutate(Prop = round(Freq / sum(Freq)*100, 2)) %>%
  mutate(Total = sum(Freq)) 
tab_IM6PENTA1D

tab_IM6PENTA2D <- svytable(~IM6PENTA2D, design = mics_design1) %>% 
  as.data.frame() %>% 
  mutate(Prop = round(Freq / sum(Freq)*100, 2)) %>%
  mutate(Total = sum(Freq)) 
tab_IM6PENTA2D 

tab_IM6PENTA3D <- svytable(~IM6PENTA3D, design = mics_design1) %>% 
  as.data.frame() %>% 
  mutate(Prop = round(Freq / sum(Freq)*100, 2)) %>%
  mutate(Total = sum(Freq)) 
tab_IM6PENTA3D 
...
tab_IM20 <- svytable(~IM20, design = mics_design1) %>% 
  as.data.frame() %>% 
  mutate(Prop = round(Freq / sum(Freq)*100, 2)) %>%
  mutate(Total = sum(Freq)) 
tab_IM20 

tab_IM21 <- svytable(~IM21, design = mics_design1) %>% 
  as.data.frame() %>% 
  mutate(Prop = round(Freq / sum(Freq)*100, 2)) %>%
  mutate(Total = sum(Freq)) 
tab_IM21 
...
tab_MN20 <- svytable(~MN20_1, design = mics_design) %>% 
  as.data.frame() %>% 
  mutate(Prop = round(Freq / sum(Freq)*100, 2)) %>%
  mutate(Total = sum(Freq)) 
tab_MN20 
...
tab_MN2 <- svytable(~MN2, design = mics_design) %>% 
  as.data.frame() %>% 
  mutate(Prop = round(Freq / sum(Freq)*100, 2)) %>%
  mutate(Total = sum(Freq)) 
tab_MN2
...
tab_MN5 <- svytable(~MN5_1, design = mics_design) %>% 
  as.data.frame() %>% 
  mutate(Prop = round(Freq / sum(Freq)*100, 2)) %>%
  mutate(Total = sum(Freq)) 
tab_MN5 
...
tab_MN3A <- svytable(~MN3A, design = mics_design) %>% 
  as.data.frame() %>% 
  mutate(Prop = round(Freq / sum(Freq)*100, 2)) %>%
  mutate(Total = sum(Freq)) 
tab_MN3A 
tab_MN3B <- svytable(~MN3B, design = mics_design) %>% 
  as.data.frame() %>% 
  mutate(Prop = round(Freq / sum(Freq)*100, 2)) %>%
  mutate(Total = sum(Freq)) 
tab_MN3B
tab_MN3C <- svytable(~MN3C, design = mics_design) %>% 
  as.data.frame() %>% 
  mutate(Prop = round(Freq / sum(Freq)*100, 2)) %>%
  mutate(Total = sum(Freq)) 
tab_MN3C
tab_MN3F <- svytable(~MN3F, design = mics_design) %>% 
  as.data.frame() %>% 
  mutate(Prop = round(Freq / sum(Freq)*100, 2)) %>%
  mutate(Total = sum(Freq)) 
tab_MN3F
tab_MN3G <- svytable(~MN3G, design = mics_design) %>% 
  as.data.frame() %>% 
  mutate(Prop = round(Freq / sum(Freq)*100, 2)) %>%
  mutate(Total = sum(Freq)) 
tab_MN3G
tab_MN3X <- svytable(~MN3X, design = mics_design) %>% 
  as.data.frame() %>% 
  mutate(Prop = round(Freq / sum(Freq)*100, 2)) %>%
  mutate(Total = sum(Freq)) 
tab_MN3X
tab_MN3NR <- svytable(~MN3NR, design = mics_design) %>% 
  as.data.frame() %>% 
  mutate(Prop = round(Freq / sum(Freq)*100, 2)) %>%
  mutate(Total = sum(Freq)) 
tab_MN3NR
...
tab_penta1_RC_1 <- svytable(~penta1_RC_1, design = mics_design1) %>% 
  as.data.frame() %>% 
  mutate(Prop = round(Freq / sum(Freq)*100, 2)) %>%
  mutate(Total = sum(Freq)) 
tab_penta1_RC_1 

tab_penta1_R_1 <- svytable(~penta1_R_1, design = mics_design1) %>% 
  as.data.frame() %>% 
  mutate(Prop = round(Freq / sum(Freq)*100, 2)) %>%
  mutate(Total = sum(Freq)) 
tab_penta1_R_1 

tab_penta1_C_1 <- svytable(~penta1_C_1, design = mics_design1) %>% 
  as.data.frame() %>% 
  mutate(Prop = round(Freq / sum(Freq)*100, 2)) %>%
  mutate(Total = sum(Freq)) 
tab_penta1_C_1 

tab_PENTA2_RC_1 <- svytable(~PENTA2_RC_1, design = mics_design1) %>% 
  as.data.frame() %>% 
  mutate(Prop = round(Freq / sum(Freq)*100, 2)) %>%
  mutate(Total = sum(Freq)) 
tab_PENTA2_RC_1 

tab_PENTA2_R_1 <- svytable(~PENTA2_R_1, design = mics_design1) %>% 
  as.data.frame() %>% 
  mutate(Prop = round(Freq / sum(Freq)*100, 2)) %>%
  mutate(Total = sum(Freq)) 
tab_PENTA2_R_1 

tab_PENTA2_C_1 <- svytable(~PENTA2_C_1, design = mics_design1) %>% 
  as.data.frame() %>% 
  mutate(Prop = round(Freq / sum(Freq)*100, 2)) %>%
  mutate(Total = sum(Freq)) 
tab_PENTA2_C_1 

tab_PENTA3_RC_1 <- svytable(~PENTA3_RC_1, design = mics_design1) %>% 
  as.data.frame() %>% 
  mutate(Prop = round(Freq / sum(Freq)*100, 2)) %>%
  mutate(Total = sum(Freq)) 
tab_PENTA3_RC_1 

tab_PENTA3_R_1 <- svytable(~PENTA3_R_1, design = mics_design1) %>% 
  as.data.frame() %>% 
  mutate(Prop = round(Freq / sum(Freq)*100, 2)) %>%
  mutate(Total = sum(Freq)) 
tab_PENTA3_R_1 

tab_PENTA3_C_1 <- svytable(~PENTA3_C_1, design = mics_design1) %>% 
  as.data.frame() %>% 
  mutate(Prop = round(Freq / sum(Freq)*100, 2)) %>%
  mutate(Total = sum(Freq)) 
tab_PENTA3_C_1 
