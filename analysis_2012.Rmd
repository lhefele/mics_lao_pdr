...
#class(mics4.ch_wm$UB1M) # NULL - why? Is not in dataset

mics4.ch_wm$AG1D_2 <- as.numeric(mics4.ch_wm$AG1D)

# Check
mics4.ch_wm %>% count(AG1D, AG1D_2) # just to check if conversion worked
...
# what class are these variable at the moment?
class(mics4.ch_wm$ethnicity) # [1] "haven_labelled" "vctrs_vctr"     "double"      
...
# 2 Khmu
# 3 Hmong
# 4 Other  --- Are the ethnicity groupings the same as in 2017?
# 9 Missing/Other
mics4.ch_wm %>% 
  mutate(ethnicity_r = recode(ethnicity, "1" = 1, "2" = 2, "3" = 3, "4" = 4, "9" = 4)) -> mics4.ch_wm
mics4.ch_wm %>% count(ethnicity, ethnicity_r)

# lang = Language group of household head
mics4.ch %>% count(ethnicity, lang) # Yes, that is very different


mics4.ch_wm %>% 
  mutate(ethnicity_r_2 = recode(ethnicity, "1" = 1, "2" = 2, "3" = 2, "4" = 2, "9" = 2)) -> mics4.ch_wm
mics4.ch_wm %>% count(ethnicity, ethnicity_r_2)
...
mics4.ch_wm %>% 
  mutate(melevel_r = recode(
    melevel, "1" = 0, "2" = 1, "3" = 2, "4" = 2, "5" = 2, "6" = 2, "9" = 0)) -> mics4.ch_wm
...
# HH&
mics4.ch_wm %>% mutate(HH6_r = recode(HH6, "1" = 1, "2" = 2, "3" = 2)) -> mics4.ch_wm
mics4.ch_wm %>% count(HH6, HH6_r)
...
mics4.ch_b1$IM3H0_time <- as.numeric(difftime(mics4.ch_b1$IM3H0_date, mics4.ch_b1$dob_ch, 
                                              units = "days"))
summary(mics4.ch_b1$IM3H0_time)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
# -365.00    0.00    0.00   17.45    6.50  688.00    5083
...
# Had a vaccination card
mics4.ch_b1 %>% 
  mutate(hepb0_RC_C = recode(hepb0_RC,
                           "0" = "1",
                           "1_Later" = "1",
                           "1_The next day" = "1",
                           "1_The same day" = "1",
...
mics4.ch_b1 %>% count(hepb0_RC_C, hepb0_RC)
...
# MN1 is received anc; 1 No, 2 yes; 9 is missing
...
mics4.ch_b1 %>% count(MN18) # Place of delivery
...
mics4.ch_b1$hepb0_RC_C <- as.factor(mics4.ch_b1$hepb0_RC_C)

# New ones

mics4.ch_b1$ethnicity_r_2 <- as.factor(mics4.ch_b1$ethnicity_r_2)
mics4.ch_b1$HH6_r <- as.factor(mics4.ch_b1$HH6_r)
...
tab <- svytable(~hepb0_RC_C, design = mics_design_b1) %>% 
  as.data.frame() %>% 
  mutate(Prop = round(Freq / sum(Freq)*100, 2)) %>%
  mutate(Total = sum(Freq)) 
tab
...
summ(svyglm(hepb0_RC_2 ~hepb0_RC_C, design = mics_design_b1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)

#hep0 within 24 hours
summ(svyglm(hepb0_RC_24 ~hepb0_RC_C, design = mics_design_b1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
...
## Multivariable analysis - Lisa



```{r variables in model}
# Variables associated with the outcome 
# having received the hepatitis B birth dose

mics4.ch_b1 %>% count(windex5) # five levels
mics4.ch_b1 %>% count(HH6)     # three levels
mics4.ch_b1 %>% count(ethnicity_r) # four levels
mics4.ch_b1 %>% count(melevel_r)   # three levels
mics4.ch_b1 %>% count(ANC)        # three levels;          NA: 1767	
mics4.ch_b1 %>% count(ANC_W)      # four levels            NA: 1767	
mics4.ch_b1 %>% count(PoB)        # three levels          NA: 1770	
mics4.ch_b1 %>% count(HL4)        # two levels
mics4.ch_b1 %>% count(HH7R)       # three levels


# How much NA per subset is this percentage-wise?

1767/(2274+2532+1767) # [1] 0.268827

# 27%



# Region and sex are NS
# Area : group into rural and urban
# Ethnicity here is not the same variable as ethnicity in the 2017 dataset
# Group as Lao-Tai vs all others
# No education vs other levels?
...
full <- svyglm(hepb0_RC_2 ~ HH6 
               ...
               + PoB 
               + HL4 
               + HH7R, design = mics_design_b1, family = quasibinomial())

summary(full)
AIC(full) # 5602.818535
...
full.i <- svyglm(hepb0_RC_2 ~ (HH6 
               ...
               + PoB 
               + HL4 
               + HH7R)^2, design = mics_design_b1, family = quasibinomial())

summary(full.i) # singularities: meaning there is a lot of overlap?!
####Cannot calculate


# Need to reduce the levels

# ethnicity (actually should be "lang"): Lao Tai vs rest
# HH6 Rural and urban
...
mics4.ch_b1 %>% count(ANC_W, ANC) # essentially the same variabel; keep ANC_W




mod <- svyglm(hepb0_RC_2 ~ HH6_r  # new variable
               + windex5 
               + ethnicity_r_2    # new variable
               + melevel_r 
               + ANC_W 
               + PoB, design = mics_design_b1, family = quasibinomial())
AIC(mod) # 5636.683274
...
# Is HH6_r important for the model?

# Remove
mod2 <- svyglm(hepb0_RC_2 ~ windex5 
               + ethnicity_r_2 
               + melevel_r 
               + ANC_W 
               + PoB, design = mics_design_b1, family = quasibinomial())
...
AIC(mod2) # 5628.772645
# final model

# other way to get model details
library(poliscidata)
fit.svyglm(mod2, digits = 3)
summ(mod2)
plot_model(mod2)

## Need to do model diagnostics
## Need to extract table data

```



```{r interactions in model?}

mod <- svyglm(hepb0_RC_2 ~ windex5 
               + ethnicity_r_2 
               + melevel_r 
               + ANC_W 
               + PoB, design = mics_design_b1, family = quasibinomial())
...
# Interactions?
modi <- svyglm(hepb0_RC_2 ~ (windex5 
               + ethnicity_r_2 
               + melevel_r 
               + ANC_W 
               + PoB)^2, design = mics_design_b1, family = quasibinomial())
summary(modi) # Cannot be estimated



# Finding interaction terms by hand - just try it out?
mod <- svyglm(hepb0_RC_2 ~ windex5 
               + ethnicity_r_2 
               + melevel_r 
               + ANC_W
               + PoB
           +windex5*ethnicity_r_2
           +windex5*melevel_r
           +windex5*ANC_W
           +windex5*PoB
           +ethnicity_r_2*melevel_r
           +ethnicity_r_2*ANC_W
           +ethnicity_r_2*PoB
           +melevel_r*ANC_W
           +melevel_r*PoB
           +ANC_W*PoB, design = mics_design_b1, family = quasibinomial())
summary(mod)
#### too much input
# INF
...
mod <- svyglm(hepb0_RC_2 ~ windex5 
           ...
           +windex5*melevel_r
           +windex5*ANC_W
           +windex5*PoB, design = mics_design_b1, family = quasibinomial())
summary(mod) # windex55:PoB3
...
mod <- svyglm(hepb0_RC_2 ~ windex5 
               ...
               + ANC_W
               + PoB
           +ethnicity_r_2*windex5
           +ethnicity_r_2*melevel_r
           +ethnicity_r_2*ANC_W
           +ethnicity_r_2*PoB, design = mics_design_b1, family = quasibinomial())
summary(mod) # windex52:ethnicity_r_22
# windex54:ethnicity_r_22
...
mod <- svyglm(hepb0_RC_2 ~ windex5 
               ...
               + PoB
           +melevel_r*windex5
           +melevel_r*ethnicity_r_2
           +melevel_r*ANC_W
           +melevel_r*PoB, design = mics_design_b1, family = quasibinomial())
summary(mod)
# Nothing
...
mod <- svyglm(hepb0_RC_2 ~ windex5 
           ...
           +melevel_r*ANC_W
           +ANC_W*PoB, design = mics_design_b1, family = quasibinomial())
summary(mod) # ethnicity_r_22:ANC_W1
...
mod <- svyglm(hepb0_RC_2 ~ windex5 
           ...
           +PoB*ANC_W
           +melevel_r*PoB, design = mics_design_b1, family = quasibinomial())
summary(mod) # ANC_W3:PoB3  # windex55:PoB3
...
mod <- svyglm(hepb0_RC_2 ~ windex5 
           ...
           +ANC_W*PoB, design = mics_design_b1, family = quasibinomial())
summary(mod)
# This would be the final model with interactions??!!



# Maybe a command for looking at the interactions?
regTermTest(mod, ~windex5+ethnicity_r_2, method="WorkingWald", df=NULL)
## IDONT KNOW
...
## Multivariable analysis - Trude


```{r hepb0 multivariate}


full <- svyglm(hepb0_RC_2 ~HH6 + windex5 + ethnicity_r + melevel_r + ANC + ANC_W + PoB + HL4 + HH7R + windex5 * ethnicity_r, design = mics_design_b1, family = quasibinomial())
...
BPM <- svyglm(formula = hepb0_RC_2 ~ HH6 + windex5 + ethnicity_r + melevel_r + 
    ANC_W + PoB + HL4 + HH7R, design = mics_design_b1, family = quasibinomial())

summ(BPM, exp = TRUE, confint = TRUE, digits = 4, vifs = TRUE)

psrsq(BPM, method = "Nagelkerke")
psrsq(BPM, method = "Cox-Snell")
psrsq(BPM, method = c("Cox-Snell", "Nagelkerke"))
...
C <- svyglm(hepb0_RC_2 ~HH6 + windex5 + ethnicity_r + melevel_r + PoB + ANC_W, design = mics_design_b1, family = quasibinomial())
...
summ(A, exp = TRUE, confint = TRUE, digits = 4, vifs = TRUE )
summ(B, exp = TRUE, confint = TRUE, digits = 4, vifs = TRUE )
summ(C, exp = TRUE, confint = TRUE, digits = 4, vifs = TRUE )
summ(D, exp = TRUE, confint = TRUE, digits = 4, vifs = TRUE )
summ(E, exp = TRUE, confint = TRUE, digits = 4, vifs = TRUE )
...
BPM <- svyglm(formula = hepb0_RC_24 ~ HH6 + windex5 + ethnicity_r + 
    melevel_r + ANC_W + PoB + HH7R, design = mics_design_b1, family = quasibinomial())
...
mics4.ch_p1 %>%
  mutate(penta1_RC_C = recode(penta1_RC,
                              "0" = 1,
                              "1" = 1,
                              "66" = 0,
                              "R_0" = 0,
...
mics4.ch_p1 %>%
  mutate(penta2_RC_1 = recode(penta2_RC,
                              ...
                              "R_1" = 1)) -> mics4.ch_p1
mics4.ch_p1 %>%
  mutate(penta2_RC_C = recode(penta2_RC,
                              "0" = 1,
                              "1" = 1,
                              "66" = 0,
                              "R_0" = 0,
...
# had a vaccination card
mics4.ch_p1 %>%
  mutate(penta3_RC_C = recode(penta3_RC,
                              "0" = 1,
                              "1" = 1,
                              "66" = 0,
                              "R_0" = 0,
...
mics4.ch_p1 %>% count(penta1_RC_C, penta2_RC_C, penta3_RC_C)
...
#new variables at least 3, info on card
mics4.ch_p1 %>% mutate(penta123_RC_C = if_else(penta1_RC_C == 1 & penta2_RC_C == 1 & penta3_RC_C == 1, 1, 0)) -> mics4.ch_p1
mics4.ch_p1 %>% count(penta123_RC_C)
...
mics4.ch_p1$penta123_RC_C <- as.factor(mics4.ch_p1$penta123_RC_C)
...
tab <- svytable(~penta123_RC_C, design = mics_design_p1) %>% 
  as.data.frame() %>% 
  mutate(Prop = round(Freq / sum(Freq)*100, 2)) %>%
  mutate(Total = sum(Freq)) 
tab
...
summ(svyglm(penta1_RC ~HH6, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta1_RC ~HH7R, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta1_RC ~windex5, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta1_RC ~HL4, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta1_RC ~ethnicity, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta1_RC ~melevel, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta1_RC ~ethnicity_r, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta1_RC ~melevel_r, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta1_RC ~PoB, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta1_RC ~ANC, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta1_RC ~ANC_W, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta1_RC ~ANC_T, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)

summ(svyglm(penta12_RC ~HH6, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta12_RC ~HH7R, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta12_RC ~windex5, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta12_RC ~HL4, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta12_RC ~ethnicity, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta12_RC ~melevel, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta12_RC ~ethnicity_r, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta12_RC ~melevel_r, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta12_RC ~PoB, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta12_RC ~ANC, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta12_RC ~ANC_W, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta12_RC ~ANC_T, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)

summ(svyglm(penta123_RC ~HH6, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta123_RC ~HH7R, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta123_RC ~windex5, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta123_RC ~HL4, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta123_RC ~ethnicity, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta123_RC ~melevel, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta123_RC ~ethnicity_r, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta123_RC ~melevel_r, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta123_RC ~PoB, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta123_RC ~ANC, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta123_RC ~ANC_W, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta123_RC ~ANC_T, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)

summ(svyglm(penta123_RC ~penta123_RC_C, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
...
summ(svyglm(penta1_C ~HH6, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta1_C ~HH7R, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta1_C ~windex5, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta1_C ~HL4, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta1_C ~ethnicity, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta1_C ~melevel, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta1_C ~ethnicity_r, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta1_C ~melevel_r, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta1_C ~PoB, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta1_C ~ANC, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta1_C ~ANC_W, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta1_C ~ANC_T, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)

summ(svyglm(penta12_C ~HH6, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta12_C ~HH7R, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta12_C ~windex5, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta12_C ~HL4, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta12_C ~ethnicity, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta12_C ~melevel, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta12_C ~ethnicity_r, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta12_C ~melevel_r, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta12_C ~PoB, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta12_C ~ANC, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta12_C ~ANC_W, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta12_C ~ANC_T, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)

summ(svyglm(penta123_C ~HH6, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta123_C ~HH7R, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta123_C ~windex5, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta123_C ~HL4, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta123_C ~ethnicity, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta123_C ~melevel, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta123_C ~ethnicity_r, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta123_C ~melevel_r, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta123_C ~PoB, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta123_C ~ANC, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta123_C ~ANC_W, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta123_C ~ANC_T, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
...
summ(svyglm(penta1_R ~HH6, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta1_R ~HH7R, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta1_R ~windex5, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta1_R ~HL4, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta1_R ~ethnicity, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta1_R ~melevel, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta1_R ~ethnicity_r, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta1_R ~melevel_r, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta1_R ~PoB, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta1_R ~ANC, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta1_R ~ANC_W, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta1_R ~ANC_T, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)

summ(svyglm(penta12_R ~HH6, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta12_R ~HH7R, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta12_R ~windex5, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta12_R ~HL4, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta12_R ~ethnicity, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta12_R ~melevel, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta12_R ~ethnicity_r, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta12_R ~melevel_r, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta12_R ~PoB, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta12_R ~ANC, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta12_R ~ANC_W, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta12_R ~ANC_T, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)

summ(svyglm(penta123_R ~HH6, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta123_R ~HH7R, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta123_R ~windex5, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta123_R ~HL4, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta123_R ~ethnicity, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta123_R ~melevel, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta123_R ~ethnicity_r, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta123_R ~melevel_r, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta123_R ~PoB, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta123_R ~ANC, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta123_R ~ANC_W, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
summ(svyglm(penta123_R ~ANC_T, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 4)
...
BPM <- svyglm(formula = penta123_RC ~ HH6 + windex5 + ethnicity_r + 
    melevel_r + HH7R + PoB, design = mics_design_p1, family = quasibinomial())
...
C <- svyglm(penta123_RC ~HH6 + windex5 + ethnicity_r + melevel_r + PoB + ANC_W, design = mics_design_p1, family = quasibinomial())
