...
mics6.ch_var <- subset(mics6.ch, select = c("HH1", "HH2", "UF1", "UF2", "UF3", "UF4", "UB2", "CAGE", "CAGED", "HH6", "HH7", "hh7r", "UF7D", "UF7Y", "UF7M", "UB1D", "UB1M", "UB1Y", "IM12A", "IM12B", "IM12C", "IM2", "IM5", "IM11", "IM15", "IM20", "IM21", "IM6H0D", "IM6H0M", "IM6H0Y", "IM6PENTA1D", "IM6PENTA2D", "IM6PENTA3D", "ethnicity", "melevel", "windex5", "HL4", "chweight", "hh6a", "hh6b", "cdisability", "UF10"))
...
mics6.ch_b1 %>% count(hepb0_RC_C, hepb0_RC)
...
mics6.ch_b1 %>% mutate(ANC_T = if_else(MN5 >= 1 & MN5 <=3 |MN5 == 98, 1,
...
mics6.ch_b1$hepb0_RC_C <- as.factor(mics6.ch_b1$hepb0_RC_C)
mics6.ch_b1$hepb0_RC_1 <- as.factor(mics6.ch_b1$hepb0_RC_1)
```
...
mics6.ch_b1 %>% count(ANC_W, MN3B, MN3C,MN3F, MN3G, MN3X)
...
tab <- svytable(~hepb0_RC_1, design = mics_design_b1) %>% 
  as.data.frame() %>% 
  mutate(Prop = round(Freq / sum(Freq)*100, 2)) %>%
  mutate(Total = sum(Freq)) 
tab 
...
summ(svyglm(hepb0_RC_2 ~PoB, design = mics_design_b1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 2)
summ(svyglm(hepb0_RC_2 ~MN2, design = mics_design_b1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 2)
summ(svyglm(hepb0_RC_2 ~ANC_W, design = mics_design_b1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 2)
summ(svyglm(hepb0_RC_2 ~ANC_T, design = mics_design_b1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 2)
...
summ(svyglm(hepb0_RC_24 ~PoB, design = mics_design_b1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 2)
summ(svyglm(hepb0_RC_24 ~MN2, design = mics_design_b1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 2)
summ(svyglm(hepb0_RC_24 ~ANC_W, design = mics_design_b1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 2)
summ(svyglm(hepb0_RC_24 ~ANC_T, design = mics_design_b1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 2)
...
BPM <- svyglm(formula = hepb0_RC_2 ~ windex5 + ethnicity_r + 
...
BPM <- svyglm(formula = hepb0_RC_24 ~ HH6 + windex5 + ethnicity_r + 
   ANC_W + PoB + hh7r, design = mics_design_b1, family = quasibinomial())
...
C <- svyglm(hepb0_RC_24 ~HH6 + windex5 + ethnicity_r + melevel_r + PoB + ANC_W, design = mics_design_b1, family = quasibinomial())
...
# at least penta 123 on Card
mics6.ch_p1 %>% count(penta1_RC_C, penta2_RC_C, penta3_RC_C)

mics6.ch_p1 %>% mutate(penta123_RC_C = if_else(penta1_RC_C == 1 & penta2_RC_C == 1 & penta3_RC_C == 1, 1, 0)) -> mics6.ch_p1
mics6.ch_p1 %>% count(penta123_RC_C)
...
mics6.ch_p1 %>% count(MN20)

mics6.ch_p1 %>% count(MN5)

mics6.ch_p1 %>% count(MN2)

mics6.ch_p1 %>% replace_with_na(replace = list(MN20 = "99")) -> mics6.ch_p1

mics6.ch_p1 %>% replace_with_na(replace = list(MN5 = "99")) -> mics6.ch_p1

mics6.ch_p1 %>% replace_with_na(replace = list(MN2 = "9")) -> mics6.ch_p1
...
```{r change anc}
mics6.ch_p1$ANC <- mics6.ch_p1$MN2


mics6.ch_p1 %>% mutate(ANC = recode(ANC,
                                     "1" = 1,
                                     "2" = 0
                                     )) -> mics6.ch_p1

# new variable place of delivery
mics6.ch_p1 %>% mutate(PoB = dplyr::recode(MN20,
                        "11" = "1",
                        "12" = "1",
                        "21" = "2",
                        "22" = "2",
                        ...
                        "36" = "3",
                        "96" = "3")) -> mics6.ch_p1
mics6.ch_p1 %>% count(MN20, PoB)


mics6.ch_p1 %>% mutate(ANC_T = if_else(MN5 >= 1 & MN5 <=3 | MN5 == 98, 1,
                                        if_else(MN5 >= 4 & MN5 <= 6, 2, 3))) -> mics6.ch_p1
mics6.ch_p1 %>% count(MN5, ANC_T)



# changing variable MN3 into categories

mics6.ch_p1 %>% count(MN3A, MN3B, MN3C, MN3F, MN3G, MN3X, MN3NR)
mics6.ch_p1 %>% count(MN2, MN3A)

mics6.ch_p1 %>% 
  unite(ANC_W, c(MN2, MN3A, MN3B), sep = "_", remove = FALSE, na.rm = FALSE) -> mics6.ch_p1

mics6.ch_p1 %>% replace_with_na(replace = list(ANC_W = "NA_NA_NA")) -> mics6.ch_p1
mics6.ch_p1 %>% replace_with_na(replace = list(ANC_W = "NA__")) -> mics6.ch_p1


mics6.ch_p1 %>% mutate(ANC_W = recode(ANC_W,
                                     "1_?_?" = 3,
                                     "1__" = 3,
                                     "1__B" = 2,
                                     "1_A_" = 1,
...
mics6.ch_p1$PoB <- as.factor(mics6.ch_p1$PoB)
mics6.ch_p1$ANC <- as.factor(mics6.ch_p1$ANC)
mics6.ch_p1$ANC_W <- as.factor(mics6.ch_p1$ANC_W)
mics6.ch_p1$ANC_T <- as.factor(mics6.ch_p1$ANC_T)
...
mics6.ch_p1$penta123_RC_C <- as.factor(mics6.ch_p1$penta123_RC_C)
```
...
summ(svyglm(penta123_RC ~penta123_RC_C, design = mics_design_p1, family = quasibinomial()), exp = TRUE, confint = TRUE, digits = 2)
...
```{r penta multivariate}


full <- svyglm(penta123_RC ~HH6 + windex5 + ethnicity_r + melevel_r + ANC + ANC_W + PoB + HL4 + hh7r, design = mics_design_p1, family = quasibinomial())
...
BPM <- svyglm(formula = penta123_RC ~ HH6 + windex5 + ethnicity_r + 
    melevel_r + ANC + PoB, design = mics_design_p1, family = quasibinomial())
